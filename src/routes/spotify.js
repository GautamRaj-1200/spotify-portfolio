...
export function configureSpotifyRouter(config) {
  ...
  router.put("/pause", async (req, res) => {
    try {
      console.log("Pause get req");
      await axios.get("https://api.spotify.com/v1/me/player/pause", null, {
        headers: { Authorization: `Bearer ${access_token}` },
      });
  ...
  router.get("/top-tracks/:artist_id", async (req, res) => {
  ...
  // 6. Play a track by URI (PUT version)
  ...
  router.get("/me/now-playing", async (req, res) => {
    try {
      const response = await axios.get(
  ...
  router.get("/", async (req, res) => {
    ...
    try {
      ...
      const topTracks = topTracksResponse.data.items.map((track) => ({
        ...
        artist: track.artists[0].name,
        uri: track.uri,
        play_url: `/spotify/play?uri=${encodeURIComponent(track.uri)}`,
      }));
      ...
      res.json({
        ...
        stop_playback_url: "/spotify/pause",
...
export default configureSpotifyRouter;
